...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (current_tab === 'result') {
    document.querySelector('.dropbtn').textContent = '수행실적';
    addIconToDropbtn();
  }
  else if (current_tab === 'confirm') {
    document.querySelector('.dropbtn').textContent = '인증현황';
    addIconToDropbtn();
  }
  else if (current_tab === 'material') {
    document.querySelector('.dropbtn').textContent = '자료실';
    addIconToDropbtn();
  }
});
...
function changePage(n) {
  const wrapper = document.querySelector("#resultListBottom");
  ...
  if (pages[n]) {
    ...
    <div class="listBox">
      ...
      <div class="listBoxBottom">
        ...
        <p>${element.post_uploadTime.substr(0,11)}</p>
      </div>
    </div>
    `;
    ...
    `;
    wrapper2.innerHTML += noticeListHtml;
    itemNumber--
  });
}
  ...
  angles.forEach((angle) => {
    if (angle.classList.contains("fa-angles-left") && currentPage !== 1) {
      ...
      else if (angle.classList.contains("fa-angles-right")) {
